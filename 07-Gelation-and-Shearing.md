...
[[5]](/07-Gelation-and-Shearing.md#5-checking-shearing) Check that the sheared gel has reached a new quasi-steady states
...
Our gelation simulations are meso-scale simlations of attractive colloidal particles in a given volume fraction (phi), with a typical colloid particles radius of ~1micron. We do not do molecular dynamics. We also do not do continuum simulations, our solvent is also simulated as particles (typically representing small groups of molecules). The simulations are non-dimensionalized, as briefly described in this document. Full details on the non-dimensionalization process can be found in the paper "[Viscosity measurement techniques in Dissipative Particle Dynamics](https://doi.org/10.1016/j.cpc.2015.05.027)" (2015).

Less that 0.5% of the simulation cost goes to simulating colloidal particles; by far the largest factor in simulation cost is the **number of solvent particles**. This is because there are far more solvent particles than colloids (for 10,000 colloidal particles at moderate volume fractions our simulations easily have ~1million solvent particles). Generally, simulations with high volume fractions of colloidal particles will therefore have LOWER simulation costs (and gel faster).
...
* a large number of colloidal particles (500-25,000; typically we use ~10,000; past work showed that simulation results do not vary significanlty for 1000, 10,000, or 100,000 particles, and so the exact number does not matter too much)
...
* The cut off distance for particle interaction "r_cut" (typically ~0.1; this was chosen by checking the radial distribution function g(r) for the peak where most particles resonate, and verified with a Max_Bond_Length calculation based on a threshold value of the attraction potential)
...
Type B particles are simulated as hard spheres that do not overlap with other particles. As noted above, our colloids are typically ~1 micron in diameter, although we define size within the simulation relativistically. Their interactions are set by a contact force (typically 10000 * kT / colloidal radius).
...
* A CSV file of static colloid-colloid pair data from each timestep (defined in our modifications with the filename "CCPairVirialLTime.csv")
...
The best practice for simulating gel formation is to try and match experimental procedures for gel formation; however, in an experiment gels are frequently left to form for very long periods of time (maybe days) because that time is cheap. In simulations we cannot usually wait that long because simulating that equivalent amount of time is very expensive. We can roughly predict the amount of simulation time that will be needed to reach a quasi-steady state (space spanning) gel from the bare-particle diffusion time (calculated from Einstein's diffusion equation, or found it existing literature). A typically simulation should run for hundreds of diffusion times (~500) to reach a steady state.

To check if this time was sufficient and the gelation simulation has reached a quasi-steady state (and formed a space spanning gel), plot the average coordination number \<Z> (AKA contact number, the number of other particles a given particle is interacting with) over time. At a quasi-steady state this number should be near constant, usually around \<Z> = 6 (octahedral structure) or sometimes \<Z> = 12 (close-packing: fcc or hcp) at very high volume fractions. The average coordination number will not reach a true plateau because there will always be some changes (i.e. "bond" breaking, "bond" formation, and lower Z values) at the gel surface, where free colloid particles behave more like a part of the Newtonian solvent. Instead it should reach a very gradual (near-plateau) slope, with minimal changes for several simulation times. You can also check the system by plotting the contact distribution (probability vs. contact number), which should produce a roughly Gaussian curve. Both of these checks are usually done in R.<br>
*NOTE: Like VMD, R is best at rendering single images rather than videos. To create high quality vides of a series of R-plots it is better to save each plot and then assemble an animation with QuickTime Player's built-in "Open Image Sequence" option.*
...
It's important to keep in mind how the interparticle attraction (Morse potential) and hydrodynamic forces interact to affect the simulation results. In particular, the shear stress (negative pressure_xy) goes up a little as the average contact number increases. As particle attraction (driven by the Morse potential) causes particles to get close together, the hydrodynamic force will increase and creates some noise (but not too much!). The noise threshold should increase as we get closer to contact number 6. The noise is not enough to affect temperature, so it’s not a concern for our simulation's success, but it is enough to require us to shear over ***long simulation times***, to iron out this noise. Nevertheless, most shearing simulations are much shorter than gelation simulations (on the order of a few days, rather than weeks).

A shearing simulation starts from the final simulation restart of a gelation simulation (i.e. a quasi-steady state gel). You must load all the gelation simulation data (i.e. you must have position information for colloid AND solvent particles) to make sure that there are no non-physical overlaps between particles when the system is initialized (i.e. no solvent particles appear in the center of a colloid particle).

Once initialized, the simulation applies a velocity equal to the shear rate at the upper and lower (+Y and -Y) boundaries of the simulation box, and the shear is then propogated through the gel via the interparticle interactions (DPD forces) using the Velocity-Verlet algorithm. This requires [the modifications we made to HOOMD-blue](/06-Modifying-HOOMDblue.md) to ensure that all particle interaction information is correctly tracked across all the periodic boundaries of the simulation box.
...
* a theta value defining the completeness of each strain (where theta = 1 is one complete "tilt" of the simulation box)
* the shear-timestep (Delta_T_Shearing) needed to acheive the desired shear rate (from: gamma_dot = tilt-distance / shear-timestep) 
